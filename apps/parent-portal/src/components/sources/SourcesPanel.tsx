...
interface SourcesPanelProps {
  ...
  onSourcesUpdated?: (sources: Source[]) => void;
...
export const SourcesPanel: React.FC<SourcesPanelProps> = ({
  ...
  const canAddSources = !isParentWorkspace || canUseAiWorkspace;
  ...
  const onSourcesUpdatedRef = useRef(onSourcesUpdated);
  onSourcesUpdatedRef.current = onSourcesUpdated;
  ...
  useEffect(() => {
    onSourcesUpdatedRef.current?.(sources);
  }, [sources]);
  ...
  const handleFileUpload = async (files: FileList | File[]) => {
    if (!canAddSources) {
      error('AI workspace access is not enabled for this child, so source uploads are disabled.');
      return;
    }
  ...
  const handleAddUrl = async (url: string) => {
    if (!canAddSources) {
      error('AI workspace access is not enabled for this child, so source creation is disabled.');
      return;
    }
    // Check source limit
      ...
      {isCollapsed ? (
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  disabled={isAtLimit || !canAddSources}
                  className={`p-2.5 rounded-lg hover:bg-muted transition-colors mb-1 ${isAtLimit || !canAddSources ? 'opacity-30 cursor-default' : ''}`}
                >
        ...
        <div className="flex flex-col h-full">
          <SourcesHeader
            ...
            onAddClick={handleOpenAddSources}
            isAtLimit={isAtLimit}
            canAddSources={canAddSources}
          />

          {!canAddSources ? (
            <div className="mx-4 mt-4 rounded-xl border border-dashed px-4 py-3 text-sm text-muted-foreground">
              This child does not currently have AI workspace access, so source uploads are read-only.
            </div>
          ) : null}
