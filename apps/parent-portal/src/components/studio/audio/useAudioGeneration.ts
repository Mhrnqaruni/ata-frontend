 ...
 * Educational Note: Manages audio overview generation with Gemini TTS.
...
export const useAudioGeneration = (projectId: string) => {
  ...
  const handleAudioGeneration = async (signal: StudioSignal) => {
    ...
    try {
      ...
      if (!ttsStatus.configured) {
        showError('Gemini API key not configured for TTS. Please add it in Admin Settings.');
        setIsGeneratingAudio(false);
        return;
      }
  ...
  const downloadAudio = async (job: AudioJob) => {
    ...
    try {
      ...
      link.href = url;
      link.download = job.audio_filename || 'audio_overview.wav';
      link.click();
