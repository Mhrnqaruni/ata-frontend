...
export const PARENT_ALLOWED_EXTENSIONS = {
  document: ['.pdf', '.txt', '.docx', '.pptx'],
  image: ['.jpeg', '.jpg', '.png', '.gif', '.webp'],
};

export const ALLOWED_EXTENSIONS = DEFAULT_ALLOWED_EXTENSIONS;

export const getAllowedExtensionsForRuntime = (isParentWorkspace: boolean) => (
  isParentWorkspace ? PARENT_ALLOWED_EXTENSIONS : DEFAULT_ALLOWED_EXTENSIONS
);

export const getAcceptedFileTypesForRuntime = (isParentWorkspace: boolean) => (
  Object.values(getAllowedExtensionsForRuntime(isParentWorkspace)).flat().join(',')
);

export const getUploadSupportText = (isParentWorkspace: boolean) => (
  isParentWorkspace
    ? 'Supported: PDF, TXT, DOCX, PPTX, JPG, JPEG, PNG, GIF, WEBP'
